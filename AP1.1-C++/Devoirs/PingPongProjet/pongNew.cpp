...
void balleGallet() {
  if ((ball.x <= galG.x + GALLET_LAR && galG.x + GALLET_LAR - ball.x <= ball.dx && ball.y >= galG.y && ball.y <= galG.y + GALLET_LONG) ||
      (ball.x >= galD.x - BALLE_TAILLE && ball.x + BALLE_TAILLE - galD.x <= ball.dx && ball.y >= galD.y && ball.y <= galD.y + GALLET_LONG))
    ball.dx *= -1;
}
