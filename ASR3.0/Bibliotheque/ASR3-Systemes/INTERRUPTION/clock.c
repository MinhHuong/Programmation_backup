...
struct tm	*time_tm;	/* local time as a structure */ // a structure used to hold time & date
int             m_delai;
...
/*
  ...
  int tm_mday; // day month  1 - 31
  int tm_mon;  // month      0 - 11
  int tm_year; // year       since 1990
  int tm_wday; // weekday (Monday, Tuesday...) 0 - 6
  int tm_yday; // 0 -> 365
  int tm_isdst;
  }
*/
...
/* forward references */
void		put_scr(), do_time(), do_date(), display_reveil(), turnOnAlarm(), turnOffAlarm(), blammo(), end_win(), exit();
int		cat_int();
unsigned	sleep();
extern FILE	*popen();
extern long	time();
...
{
  long	ticks;		/* seconds since epoch */ // epoch : January 1, 1970
  int	old_day = -1;	/* previous julian day */

  if(argc == 2)
    {
      m_delai = atoi(argv[1]);    /* seconds */
    }
  else
    {
      m_delai = 0;
  ...
  signal(SIGALRM, turnOnAlarm );

  clear();			/* setup */ //clear the screen
  noecho();                   // disable echo for current screen
  ...
  while (TRUE) {
    ...
    ticks = time( (long *)NULL ); // calculat the current time & encodes it into time_t format
                                  // time_t : type suitable for storing a calendar time
                                  // parameter : the pointer of type time_t, where the seconds value is stored

    strncpy(time_buf, ctime(&ticks), 24); // copy characters from string
    // dest   ,  source      , nb de caracteres a copier                               
    // ctime(&ticks) : string representing the localtime based on &tick
    // format : www mmm dd hh:mm:ss yyyy
    time_tm = localtime(&ticks);
    // locatime(time_tm ...) : uses the time pointed by ... to fill a tm structures with the values that representing the correponding local time 
...
/*
** Turn on alarm after m_delai seconds
*/
void turnOnAlarm()
{
  display_reveil();
  signal(SIGALRM, turnOffAlarm);
  alarm(STAY_ON);
}

/*
** Turn off alarm after STAY_ON seconds
*/
void turnOffAlarm()
{
  char  clean[2] = " ";
  ...
  do_date();
  do_time();
  put_scr(MSG_LINE, clean);
  signal(SIGALRM, turnOnAlarm);
  alarm(m_delai);  
}

/*
** Display the word "Reveil" on screen
*/
void display_reveil()
{
  char msg[7] = "Reveil";
  msg[6] = '\0';
  put_scr(MSG_LINE, msg);
}
     ...
     char	*what;
{
  ...
  strcat(buf, "'"); // append the string param2 to the end of the string param1
  strcat(buf, what);
  strcat(buf, "'");
