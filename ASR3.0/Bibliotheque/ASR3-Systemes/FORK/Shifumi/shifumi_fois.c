...
int main(int argc, char *argv[])
{
  ...
  for(i = 0 ; i < nb_joueurs*fois ; i++)
  {
	  printf("Joueur %d - Symbol %d : \n", i/fois + 1, nb_symbol[i]);
  }
...
void calculerPoint(int pidArray[], int pointArray[], int symbolArray[], int size, int fois)
{
  ...
  for(f = 0 ; f < fois ; f++)
  {
	  for(j = 0 ; j < size ; j++)
	  {
		  for(i = 0 ; i < size ; i++)
		  {
			  if(symbolArray[j*fois+f] != symbolArray[i*fois+f])
			  {
				  if(estGane(symbolArray[j*fois+f], symbolArray[i*fois+f])) pointArray[j*fois+f]++;
			  }
		  }
	  }
  }
