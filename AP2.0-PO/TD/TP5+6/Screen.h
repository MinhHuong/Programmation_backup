...
class Screen
{
 private:
  ...
  char my_screen[XMAX][YMAX];   
  // écran virtuel (zone) pour dessiner
	
  char my_colPen;               
  // couleur courante (du crayon) pour le dessin
 
 public:
  Screen();
  void clear();
  // efface l'écran (zone) de dessin

  void refresh();    
  // reaffiche l'écran (zone) de dessin
  ...
  void print_points_circle(const Point & p);
